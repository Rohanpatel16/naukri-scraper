...
def _parse_listing_page(html: str, base_url: str) -> List[Dict[str, Any]]:
    """Parse a Wellfound listing page and extract company + job data accurately."""
    soup = BeautifulSoup(html, "html.parser")
    companies: List[Dict[str, Any]] = []

    # Each company has a link /company/...
    ...
    for a in company_links:
        ...
        # Walk up to find the enclosing card container that holds jobs for this company
        card = a
        found_card = None
        for _ in range(7):
            if not card.parent:
        ...
        if not found_card:
            # Check if parent card has at least some jobs or info
            found_card = card
        ...
        if found_card:
            ...
            for jlink in job_links:
                ...
                # Parent details for salary / location
...
class WellfoundScraper:
    ...
    async def _scrape_page(self, page: Page, url: str) -> str:
        ...
        try:
            ...
            await page.wait_for_timeout(3500)  # Allow JS components to mount
    ...
    async def _async_scrape(
        ...
        """Main async scraper coordinator."""
        async with async_playwright() as p:
            ...
            # Block images and unnecessary media for maximum speed
        ...
        # Dynamic live enrichment for Website + LinkedIn
    ...
    def scrape(
        ...
        try:
            return asyncio.run(self._async_scrape(search_url, max_pages))
        except RuntimeError:
            loop = asyncio.get_event_loop()
            return loop.run_until_complete(self._async_scrape(search_url, max_pages))
